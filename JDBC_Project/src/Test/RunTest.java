...
public class RunTest {
	/*
	 * JDBC를 사용하기위해서는 자바 프로젝트에 jdbc드라이버를 추가해줘야한다.
	 * 
	 * *JDBC용 객체
	 ...
	 * 3) Statement 생성 : Connection 객체를 이용해서 생성(sql문 실행및 결과를 받는 객체)
	...
	public static void main(String[] args) {
		...
		// 실행할 sql문(완전한 상채로 만들어두기)(맨뒤에 세미콜론 없어야된다!!!!!!!!!)
			//	String sql = "INSERT INTO TEST VALUES(1, '김효영', SYSDATE)";
		...
		try {
			...
			// 4, 5) sql문 전달하면서 실행 후 결과 받기(행 수)
			result = stmt.executeUpdate(sql);
			// 내가 실행할 sql문이 dml(INSERT, UPDATE, DELETE)문 -> stmt.executeUpdate(sql문) : int
			// 내가 실행할 sql문이 SELECT문 -> stmt.executeQurey(SELECT문) : ResultSet
		...
		*/
		...
		// SELECT문 => 결과 ResultSet(조회된 데이터가 담겨있다) => ResutlSet으로부터 데이터 추출
		...
		try {
			...
			// rset.next() => rest의 다음행이 들어있는지 없는지 확인 후 결과를 반환 + 한행을 내려줍니다.
