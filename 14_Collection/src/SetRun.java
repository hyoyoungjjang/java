...
public class SetRun {
	public static void main(String[] args) {
		// Object에 equals() => 두 객체의  "주소값"을 비교해서 일치하면 true/일치하지않으면 false를 반환
		...
		// String에 equals() 오버라이딩 => "실제 담긴 문자열"을 가지고 동등비교를 진행하여 일치하면 true/ 일치하지않으면 false 반환
		// String에 hashCode() 오버라이딩 => "실제 담긴 문자열"을가지고 10진수의 형태로 만들어서 반환
		
		
		HashSet hs2 = new HashSet();
		hs2.add(new Student("공유", 43, 100));
		hs2.add(new Student("김효영", 22, 95));
		hs2.add(new Student("홍길동", 55, 15));
		hs2.add(new Student("공유", 43, 100));
		...
		//Student에 equals() 오버라이딩 => "실제 각 필드에 담긴 데이터"들이 일치하면 true/일치하지않으면 false를 반환
		//Student에 hashCode() 오버라이딩 => "실제 각 필드에 담긴 데이터"들이 일치하면 동일한 10진수 반환
		System.out.println(hs2);
		
		//hs2.get(1); // 인덱스의 개념도 없고  get메소드 자체가 정의되어있지 않음!! -> 한개식 무작위로 가져올 수 있음
		...
		// ArrayList담는 방법 1 : ArrayList생성 후 addAll메소드를 이용해서 통채로 추가
