...
public class MapRun {
	public static void main(String[] args) {
		...
		//1. put(key, V value) : Map에 키 벨류 세트로 추가시켜주는 메소드
		hm.put("다이제", new Snack("초코맛", 2500));
		...
		// 동일한 키값으로 다시 추가하는 경우 value값이 덮어씌어짐!! (중복된 키값 공존할 수 없음! 키값이 식별자 역할을 함)
		...
		//4. replace(k key, v value) => 해당 키값을 찾아서 다시 전달한 Value값으로 수정시켜주느 메소드
		hm.replace("먹태깡", new Snack("엄첨 짠맛", 600));
		System.out.println(hm);
...
//		}
		...
		// 바로 Terator를 호출할 수 없음 왜? -> Iterator는 List계열 또는 Set계열에서만 가능하기 때문
...
//		prop.put("다이제", new Snack("초코맛", 2500));
...
//		prop.put("새우깡", new Snack("짠맛", 500));
		...
		try {
				...
				// ClassCastException
				//내부적으로 store실행시 proerties에 담겨있는 키+벨류 세트들을 String으로 형변환해서 출력함
		 ...
		 * => 모두 문자열이기 때문에 개발자가 아닌 일반인관리자가 해당 문서를 수정하기 쉽지 않다.
		 * 
