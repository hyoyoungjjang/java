...
//		p2.printParent();
//		((Child1)p2).printChild1(); //강제형변환을 하면 호출 가능 !!(다우캐스팅)
//		//p2 래퍼런스로 지금 당장은 parent에만 접근이 가능하지만
...
//		((Child2)arr[3]).printChild2();
		...
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] instanceof Child1) { // 해당 참조변수 실제 Child1을 참조하고 있니?(arr[i]가 Child1공간 사용가능해?)
