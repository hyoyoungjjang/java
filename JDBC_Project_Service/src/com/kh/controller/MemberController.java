...
//Controller : View를 통해서 사용자가 요청하는 기능에 대해서 처리하는 담당
//				해당 메소드로 전달된 데이터[가공처리후] dao로 전달하며 호출
//				dao로부터 반환받은 결과 성공인지 실패인지 판단후 응답화면 결정(view메소드 호출)
public class MemberController {
	...
	public void updateMember(String userID, String userPwd, String email, String phone, String address) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess("성공적으로 회원 정보 변경하였습니다.");
		}else {
	...
	public void deleteMember(String userId) {
		...
		if(result > 0) {
			new MemberMenu().displaySuccess("회원 삭제에 성공했습니다.");
		} else {
			new MemberMenu().displayFail("회원 삭제에 실패했습니다.");
		}
	}
	...
	public void selectByUserId(String userId) {
		...
		if(m == null) {
			new MemberMenu().displayNoDate(userId + "에 해당하는 조회결과가 없습니다.");
		} else {
