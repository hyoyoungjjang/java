...
public class TCPServer {
	 ...
	 * - 서버, 클라이언트간에 1:1 소켓통신
	 * - 데이터를 교환하기에 앞서 서버, 클라이언트 연결이 되어야한다.(서버가 먼저 실행되어 클라이언트에 요청을 기다려)
	 * - 신뢰성있는 데이터 전달가능
	 * 
	 * *Socket
	 * -프로세스간의 통신을 담당
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 4) 연결요청이 오면 => 요청 수락 후 클라이언트와 통신할 수 있는 서버측 소켓객체를 생성
			...
			while(true) {
				...
				pw.println(sendMessage); //클라이언트 출력
				pw.flush(); //현제 스트림에 있는 데이터를 강제로 내보내는 메소드
