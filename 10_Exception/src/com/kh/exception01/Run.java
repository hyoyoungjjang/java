...
public class Run {
	/*
	 * * 에러 종류
	 * - 시스템에러 : 컴퓨터의 오작동으로 발생하는 애러 => 소스코드로 해결이 안됨 => 심각한 에러
	 * - 컴파일에러 : 소스코드 문법상의 오류 => 빨간줄로 애초에 오류로 알려준다. => (개발자의 실수) => 발견과 해결이 쉽다.
	 * - 런타임에러 : 코드사응로 문제가 없는데 프로그램 실행 중에 발생하는 에러(사용자의 실수일 수 있고 개발자가 예측 가능하지만 처리를 안해놓은 에러)
	 * - 논리에러  : 문법적으로도 문제없고 실행했을 때도 문제는 없지만 프로그램의도와 맞지않은 동작이나 결과가 나타는 에러
	 * 
	 * 컴파일에러, 런타임에러, 논리에러 같은 개발자가 예측하여 수정할 수 있는 에러들을 가지고 작업
	 * => 이런것들을  "예외"라고 함 !!=> Exception
	 * 
	 * 이런 "예외"가 발생했을 경우에 대해서 처리하는 방법을 예외처리라고 함!
	 * 
	 * *예외처리를 하는 목적
